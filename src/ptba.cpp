  ...
  std::visit(
      [&](auto const &log_data) {
        ...
        if constexpr (std::is_same_v<variant, log_data::NotEnoughReads>) {
          logger::debug("Transcript {}, window size {}, replicate {}, window "
                        "{} (bases {}-{}) is skipped by initial filtering: "
                        "reads ({}) < min_filtered_reads ({}).",
                        transcript.name, window_size, replicate_index + 1,
          ...
          logger::debug("Transcript {}, window size {}, replicate {}, window "
                        ...
                        transcript.name, window_size, replicate_index + 1,
                        window_index + 1, window.start_base + 1,
                        window.start_base + window_size, log_data.bases,
                        args.min_bases_size());
        } else if constexpr (std::is_same_v<variant,
                                            log_data::AllZeroEigenGaps>) {
          logger::debug(
              "Transcript {}, window size {}, replicate {}, window {} (bases "
              "{}-{}) is skipped because all eigengaps are zero.",
              transcript.name, window_size, replicate_index + 1,
              window_index + 1, window.start_base + 1,
          ...
          logger::debug(
              "Transcript {}, window size {}, replicate {}, window {} (bases "
              "{}-{}) is skipped because the number of eigengaps ({}) or the "
              "max number of clusters ({}) is less than 1",
              transcript.name, window_size, replicate_index + 1,
              window_index + 1, window.start_base + 1,
              window.start_base + window_size, log_data.eigengaps,
              args.max_clusters());
        } else if constexpr (std::is_same_v<variant, log_data::Permuting>) {
          std::string message =
              std::format("Transcript {}, window size {}, replicate {}, window "
                          "{} (bases {}-{}):",
                          transcript.name, window_size, replicate_index + 1,
                          window_index + 1, window.start_base + 1,
                          window.start_base + window_size);
