...
template <typename T> struct is_het_allocator : std::false_type {};
...
template <typename T> using has_pointer = typename T::pointer;
template <typename T> using has_const_pointer = typename T::const_pointer;
template <typename T> using has_void_pointer = typename T::void_pointer;
template <typename T>
using has_const_void_pointer = typename T::const_void_pointer;
template <typename T> using has_difference_type = typename T::difference_type;
template <typename T>
using has_lines_difference_type = typename T::lines_difference_type;
template <typename T> using has_size_type = typename T::size_type;
template <typename T> using has_lines_size_type = typename T::lines_size_type;
template <typename T>
...
template <typename T> using has_is_always_equal = typename T::is_always_equal;
template <typename T> using has_is_packed = std::bool_constant<T::is_packed>;
template <typename T> using has_construct = typename T::construct;
template <typename T> using has_destroy = typename T::destroy;
template <typename T> using has_max_size = typename T::max_size;
template <typename T>
...
template <typename HetAlloc> struct allocator_traits {
  ...
  allocate(HetAlloc &alloc, lines_size_type count, Sizes... sizes) {
    return check_and_run(
        ...
        [&] { return alloc.allocate(count, sizes...); }, sizes...);
  }
  ...
  allocate(HetAlloc &alloc, Sizes... sizes, lines_size_type count,
    ...
    return check_and_run(
        std::index_sequence_for<Sizes...>(),
        [&] { return alloc.allocate(sizes..., count, hint); }, sizes...);
  }

  template <typename... Sizes>
  static inline void deallocate(HetAlloc &alloc, first_pointer p,
                                lines_size_type count, Sizes... sizes) {
    check_and_run(
        std::index_sequence_for<Sizes...>(),
  ...
  static inline void construct(HetAlloc &alloc, T *p, Args &&...args) {
  ...
  template <typename... Sizes>
  static constexpr lines_size_type size_of_line(const HetAlloc &alloc,
                                                Sizes... sizes) noexcept {
    return check_and_run(
        std::index_sequence_for<Sizes...>(),
        [&] { return alloc.size_of_line(sizes...); }, sizes...);
  }

  template <std::size_t Idx, typename... Sizes>
  static constexpr lines_difference_type offset_of(const HetAlloc &alloc,
                                                   Sizes... sizes) noexcept {
  ...
  first_pointer_of(const HetAlloc &alloc, first_pointer first,
    ...
    return check_and_run(
        std::index_sequence_for<Sizes...>(),
        [&] {
          return alloc.template first_pointer_of<Idx>(first, line_index,
                                                      sizes...);
        },
        sizes...);
  }
...
private:
  ...
  static inline decltype(auto) check_and_run(std::index_sequence<Idx...>,
                                             Fun fun, Sizes...) {
