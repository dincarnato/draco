...
std::vector<RingmapData> RingmapData::split_into_windows(
  ...
  auto windows_iter = std::begin(windows);
  auto splitted_ringmaps_iter = std::begin(splitted_ringmaps);
  auto const splitted_ringmaps_end = std::end(splitted_ringmaps);
  for (; splitted_ringmaps_iter < splitted_ringmaps_end;
       ++splitted_ringmaps_iter, ++windows_iter) {
    auto &&window = *windows_iter;
    auto &&ringmap = *splitted_ringmaps_iter;
    auto rows_iter = std::ranges::begin(rows);
    auto const end_rows = std::ranges::end(rows);
    ...
    std::vector<std::size_t> last_step_indices;
    for (; rows_iter < end_rows; ++rows_iter) {
      auto &&row = *rows_iter;
      if (row.begin_index() >= window.end_index) {
        break;
      }

      if (row.end_index() <= window.begin_index or
          calc_modifications_intersection(row, window) == 0) {
        continue;
      }
