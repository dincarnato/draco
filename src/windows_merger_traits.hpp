...
struct WindowsMergerTraits {
  ...
  static constexpr bool is_window_baselike_v =
      nostd::is_detected_convertible_v<const weight_type&, weight_getter_t,
                                       T>and
          nostd::is_detected_convertible_v<const coverage_type&,
  ...
  template <typename T>
  using begin_index_getter_t = decltype(std::declval<const T&>().begin_index());
  ...
  template <typename T>
  using begin_index_setter_t = decltype(
      std::declval<T&>().set_begin_index(std::declval<bases_size_type>()));

  template <typename T>
  static constexpr bool is_windowlike_v = nostd::is_detected_exact_v<
      clusters_size_type, clusters_size_getter_t, T>and
  ...
  template <typename T>
  struct is_reshaper_arg : std::false_type {};
  ...
  template <typename T>
  struct is_reserve_reshaper : std::false_type {};
  ...
  template <typename T>
  struct is_resize_reshaper : std::false_type {};
...
template <>
struct WindowsMergerTraits::is_rvalue_window_baselike_reference<WindowsMergerWindowBase&&, void>
    : std::true_type {};
