...
void WindowsMergerWindow::assign_from_accessor(Accessor &&accessor) noexcept(
    false) {
...
inline auto WindowsMergerWindow::end_index() const noexcept -> bases_size_type {
  assert(_bases.size() <=
...
inline auto WindowsMergerWindow::size() const noexcept -> bases_size_type {
  assert(_bases.size() <= std::numeric_limits<bases_size_type>::max());
  return static_cast<bases_size_type>(_bases.size());
}

inline bool WindowsMergerWindow::empty() const noexcept {
  return _bases.empty();
}
...
inline auto WindowsMergerWindow::coverages() noexcept -> coverages_accessor {
...
inline auto WindowsMergerWindow::linear_weights() const noexcept
...
inline auto WindowsMergerWindow::front() const noexcept -> base_type const & {
  assert(not _bases.empty());
  return _bases.front();
}

inline auto WindowsMergerWindow::back() const noexcept -> base_type const & {
  assert(not _bases.empty());
  return _bases.back();
}

inline auto WindowsMergerWindow::front() noexcept -> base_type & {
  assert(not _bases.empty());
  return _bases.front();
}

inline auto WindowsMergerWindow::back() noexcept -> base_type & {
  assert(not _bases.empty());
  return _bases.back();
}
...
inline auto WindowsMergerWindow::emplace_back(Args &&...args) noexcept(false)
    -> base_type & {
...
inline void WindowsMergerWindow::push_back(Base &&base) noexcept(false) {
