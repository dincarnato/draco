...
template <typename Matrix>
auto RingmapMatrixRowIterator<Matrix>::operator++() noexcept -> self & {
  ++row;
  return *this;
}

template <typename Matrix>
auto RingmapMatrixRowIterator<Matrix>::operator++(int) noexcept -> self {
  auto other = *this;
  ++row;
  return other;
}
...
template <typename Matrix>
auto RingmapMatrixRowIterator<Matrix>::operator-(self const &rhs) const noexcept
    -> difference_type {
...
template <typename Matrix>
bool RingmapMatrixRowIterator<Matrix>::operator==(
    const RingmapMatrixRowIterator &other) const noexcept {
