...
template <typename Mat, ArmaIteratorDirection direction>
auto ArmaIteratorHandler<Mat, direction>::begin() const noexcept -> iterator {
  return {matrix};
}

template <typename Mat, ArmaIteratorDirection direction>
auto ArmaIteratorHandler<Mat, direction>::end() const noexcept -> iterator {
  return {matrix, 0};
}

template <typename Mat, ArmaIteratorDirection direction>
std::size_t ArmaIteratorHandler<Mat, direction>::size() const noexcept {
