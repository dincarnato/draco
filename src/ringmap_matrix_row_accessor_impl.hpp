...
template <typename Matrix>
auto
RingmapMatrixRowAccessor<Matrix>::operator=(
...
template <typename Matrix>
auto
RingmapMatrixRowAccessor<Matrix>::begin() const noexcept -> iterator {
  assert(matrix);
  return {*matrix, row, 0};
}
...
template <typename Matrix>
auto
RingmapMatrixRowAccessor<Matrix>::end() const noexcept -> iterator {
  assert(matrix);
...
template <typename Matrix>
std::size_t
RingmapMatrixRowAccessor<Matrix>::sum() const noexcept {
  return ranges::accumulate(begin(), end(), 0u);
}

template <typename Matrix>
double
RingmapMatrixRowAccessor<Matrix>::mean() const noexcept {
  assert(matrix);
  return static_cast<double>(sum()) / matrix->bases;
}

template <typename Matrix>
arma::rowvec RingmapMatrixRowAccessor<Matrix>::operator*(
    const arma::mat& eigenVectors) const noexcept(false) {
  assert(matrix);
...
template <typename T>
T
RingmapMatrixRowAccessor<Matrix>::convTo() const noexcept(false) {
  assert(matrix);
...
void
RingmapMatrixRowAccessor<Matrix>::shuffle(URBG&& g) const
...
RingmapMatrixAccessor<Matrix>
    RingmapMatrixRowAccessor<Matrix>::operator[](unsigned index) const
    noexcept {
  assert(matrix);
  return {row, index};
}
...
void
RingmapMatrixRowAccessor<Matrix>::assign_from_accessor(
...
template <typename Accessor>
void
RingmapMatrixRowAccessor<Matrix>::move_from_accessor(Accessor&& accessor) const
    noexcept(std::is_nothrow_move_assignable_v<ringmap_matrix::row_type>) {
...
template <typename Matrix>
auto
RingmapMatrixRowAccessor<Matrix>::operator=(value_type const& row) const
    noexcept(false) -> RingmapMatrixRowAccessor const& {
  assign_from_row(row);
  return *this;
}
...
template <typename Matrix>
auto
RingmapMatrixRowAccessor<Matrix>::operator=(value_type&& row) const
    noexcept(false) -> RingmapMatrixRowAccessor const& {
...
void
RingmapMatrixRowAccessor<Matrix>::assign_from_row(Row&& row) const
    noexcept(false) {
...
template <typename Matrix>
void
RingmapMatrixRowAccessor<Matrix>::set_begin_index(
...
template <typename Matrix>
void
RingmapMatrixRowAccessor<Matrix>::set_end_index(
    ringmap_matrix::base_index_type value) noexcept {
  row->end_index = value;
}
...
bool
operator<(RingmapMatrixRowAccessor<ML> const& lhs,
...
template <typename M>
bool
operator<(RingmapMatrixRowAccessor<M> const& lhs,
...
template <typename M>
bool
operator<(ringmap_matrix::row_type const& lhs,
