...
template <typename>
struct WindowsMergerWindowAccessorCoveragesAccessor;
template <typename>
...
struct WindowsMergerWindowAccessor {
  ...
  void push_back(Base&& base) const noexcept(false);
...
private:
  ...
  auto merger_get_window_first_pointer(windows_size_type window_index) const
      noexcept;

  template <std::size_t Index>
  auto merger_get_coverage_first_pointer(windows_size_type window_index) const
      noexcept;
