...
struct vec2d_init_parts : vec2d_build_parts_common<vec2d_init_parts, Ts...> {
  ...
  template <typename T>
  using append_t = vec2d_init_parts<Ts..., T>;
...
template <typename T>
struct vec2d_rt_part_construct_default : T {
...
template <typename T>
struct vec2d_rt_part_uninitialized : T {
  static_assert(is_vec2d_rt_part_generic_construct_base_v<T>);
...
template <typename T>
vec2d_rt_part_uninitialized(T &&)->vec2d_rt_part_uninitialized<std::decay_t<T>>;
...
struct vec2d_rt_part_construct : T {
  ...
  inline auto
  new_appended(Args&&... args) noexcept(
  ...
  inline auto
  new_appending_tuples(NewArgsTuples&&... new_args_tuples) noexcept(
...
private:
  ...
  inline auto
  new_appending_tuples_impl(
      std::index_sequence<Idx...>,
      NewArgsTuples&&... new_args_tuples) noexcept(std::
