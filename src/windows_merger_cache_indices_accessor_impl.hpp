...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::index() const noexcept
    -> windows_size_type {
  return line_index;
}
...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::size() const noexcept
    -> windows_size_type {
...
template <typename Merger>
bool
WindowsMergerCacheIndicesAccessor<Merger>::empty() const noexcept {
  return size() == 0;
}

template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::front() const noexcept -> reference {
  assert(size() > 0);
...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::back() const noexcept -> reference {
  assert(size() > 0);
...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::begin() const noexcept -> iterator {
  return merger->template get_indices_first_pointer<1>(line_index);
}

template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::end() const noexcept -> iterator {
  return merger->template get_indices_first_pointer<1>(line_index) + size();
}

template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::rbegin() const noexcept
    -> reverse_iterator {
...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::rend() const noexcept
    -> reverse_iterator {
  return ranges::reverse_iterator<iterator>(begin());
}

template <typename Merger>
auto WindowsMergerCacheIndicesAccessor<Merger>::
operator[](windows_size_type index) const noexcept -> reference {
  assert(index < size());
...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::emplace_back(
...
template <typename Merger>
void
WindowsMergerCacheIndicesAccessor<Merger>::push_back(
...
template <typename Merger>
void
WindowsMergerCacheIndicesAccessor<Merger>::resize(windows_size_type value) const
    noexcept(false) {
...
template <typename Merger>
void
WindowsMergerCacheIndicesAccessor<Merger>::clear() const noexcept {
  static_assert(std::is_trivially_constructible_v<windows_size_type>);
  static_assert(std::is_trivially_destructible_v<windows_size_type>);
...
template <typename Merger>
auto
WindowsMergerCacheIndicesAccessor<Merger>::operator=(
...
operator=(WindowsMergerCacheIndicesLine const& line) const noexcept(false)
    -> self const& {
...
auto
WindowsMergerCacheIndicesAccessor<Merger>::
operator=(WindowsMergerCacheIndicesLine&& line) const noexcept(false)
    -> self const& {
...
void
WindowsMergerCacheIndicesAccessor<Merger>::assign_from_accessor(
...
void
WindowsMergerCacheIndicesAccessor<Merger>::assign_from_line(Line&& line) const
    noexcept(false) {
  assert(line_index < merger->_lines_size);
