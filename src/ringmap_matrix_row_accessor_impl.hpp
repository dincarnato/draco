...
template <typename Matrix>
constexpr ringmap_matrix::base_index_type
RingmapMatrixRowAccessor<Matrix>::window_begin_index() const noexcept {
  assert(matrix);
  return row->window_begin_index();
}

template <typename Matrix>
constexpr ringmap_matrix::base_index_type
RingmapMatrixRowAccessor<Matrix>::window_end_index() const noexcept {
  assert(matrix);
  return row->window_end_index();
}
...
std::enable_if_t<std::is_same_v<std::decay_t<Matrix1>, std::decay_t<Matrix2>>,
                 bool>
RingmapMatrixRowAccessor<Matrix1>::has_same_indices(
    RingmapMatrixRowAccessor<Matrix2> const &other) const noexcept {
  return row->has_same_indices(*other.row);
}
