...
static void test2()
{
    ...
    auto result = getBestMatchingIndices(std::begin(data1), std::end(data1), std::begin(data2), std::end(data2), [](auto a, auto b) {
                return std::abs(a - b);
            }, std::greater<int>());
    if(not std::equal(std::begin(expectedResult), std::end(expectedResult), std::begin(result), std::end(result)))
        std::exit(-1);
}
