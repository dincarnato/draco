...
arma::Mat<unsigned> readRingmapFile(const char *filename) {
...
int main(int argc, const char *argv[]) {
  ...
  /* Row-wise check */
  {
    ...
    for (auto &&row : matrix.rows()) {
      ...
      assert(
          row.mean() ==
          arma::mean(arma::conv_to<arma::mat>::from(armaMatrix).row(rowIndex)));
  ...
  /* Col-wise check */
  {
    ...
    for (auto &&col : matrix.cols()) {
      ...
      assert(
          col.mean() ==
          arma::mean(arma::conv_to<arma::mat>::from(armaMatrix).col(colIndex)));
